...
export function initializeFuzzyArea({
  ...
  const fuzzyareaHandler = (containerEle, textarea) => {
    ...
    const ro = new ResizeObserver(() => {
      mirroredEle.style.width = `${textarea.clientWidth + 2 * borderWidth
        }px`;
      mirroredEle.style.height = `${textarea.clientHeight + 2 * borderWidth
        }px`;
    });
    ro.observe(textarea);
  ...
  const __init = () => {
    ...
    if (waitForElement) {
      ...
      fuzzyareaHandler(__getBySelector(_containerSelector), __getBySelector(_textareaSelector));
    }
  }
